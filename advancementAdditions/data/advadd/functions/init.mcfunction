#Stat Based
scoreboard objectives add horizon_death deathCount
scoreboard objectives add advadd_health health
scoreboard objectives add painting_check minecraft.used:minecraft.painting

#Dummy
scoreboard objectives add advadd_raidadder dummy
scoreboard objectives add just_keep_going dummy
scoreboard objectives add underwater_secs dummy
scoreboard objectives add advadd_wiz_break dummy
scoreboard objectives add advadd_chorus dummy
scoreboard objectives add drag_v_drag dummy
scoreboard objectives add eventhorizon dummy
scoreboard objectives add advadd_castaway dummy
scoreboard objectives add let_me_out dummy
scoreboard objectives add current_time dummy
scoreboard objectives add day_count dummy


#Start Function timers
function advadd:second_timer
function advadd:ten_sec_timer